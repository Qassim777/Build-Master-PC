...
<?php if (isLoggedIn()): ?>
  ...
  <body>
    <form action="Admin_Dashboard_Search.php" method="get">
      <div class="cart" style="display: flex; justify-content: center; align-items: center;">
        ...
        <button class="update-image">Search</button>
      </div>
    </form>
    ...
    try {
      ...
      if (!empty($searchTerm)) {
        ...
        $query = "SELECT * FROM product WHERE Pro_Name LIKE '%" . mysqli_real_escape_string($database, $searchTerm) . "%'";
        ...
        if (mysqli_num_rows($result) == 0) {
          ...
          while ($row = mysqli_fetch_assoc($result)) {
            echo '<div class="pc-container style=  height: 400px; ">';
            ...
            if (isset($row['picture'])) {
              ...
              echo '<a href="../../php/product_page.php" name="dataInput" onclick="setProdcutNum(' . $row['ProductID'] . ')">';
              echo '<div class="img-continer">';
              echo '<img src="" class="pc-img">'; // Default image if 'picture' is not set
              echo '</div>';
            ...
            echo '</div>';
            echo '<div style="margin-bottom: 15px;" class="VAT">VAT not included</div>';
            echo '<div class="cart">';
            echo '<form action="Admin_Dashboard_Modify3.php" method="post">';
            echo '<input type="hidden" name="ProductID" id="ProductID" value="' . $row['ProductID'] . '">';
            echo '<button type="submit" class="cart-button" value="' . $row['ProductID'] . '"><img class="shopping-cart-img" alt="modify product icon" src="../../Image/modify.png"></button>';
            echo '</form>';
            echo '</div>';
            echo '</div>';
          }
